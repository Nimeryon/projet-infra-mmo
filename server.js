...
function sendMail(email, hash, pseudo) {
    let url = `https://mmo-project.herokuapp.com/validate_account.html?hash=${hash}`;
...
io.on('connection', function (socket) {
    ...
    socket.on('account validation', function (hash) {
        db.account.updateOne({ activationHash: hash }, { $set: { active: true }, $unset: { "activationHash": "" } }, function (err, res) {
    ...
    socket.on('signin request', function (data) {
        db.account.find({ $or: [{ email: data.email }, { username: data.username }] }, function (err, res) {
            ...
            if (res != null) {
                ...
                let activationHash = generateString(128);
                db.account.insertOne({ email: data.email, username: data.username, password: data.password, active: false, activationHash: activationHash });
                sendMail(data.email, activationHash, data.username);
